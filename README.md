...
- 本地团队和避雷记录按 AstrBot 会话严格隔离；团队支持 10 人或 25 人规格、五人小队、老板标记及可配置报名限制。
...
- JX3API 查询使用内存与 SQLite 两级 JSON 缓存，HTML 查询图片使用本地文件缓存；缓存时间、接口内存容量、SQLite 接口缓存条数及图片缓存总容量均可在 WebUI 配置。
- 内置 98 个中文触发指令和 52 个页面片段，通过公共布局与样式在本地组装为完整 HTML，并附带通用、沙盘、门派/心法和奇遇图标资源。
...
| 本地 SQLite | `core/sqlite.py`、`core/team.py`、`core/team_rules.py`、`core/bilei_data.py`、`core/cache.py` | 团队、报名规则、会话配置、别名、避雷记录、事件订阅、接口缓存及图片缓存索引 |
...
| `tls_verify` | `bool` | `true` | 验证 JX3API、JX3BOX 和事件推送连接的 TLS 证书；仅建议在受信任的调试环境中临时关闭 |
| `team_management.view_output` | `string` | `图片` | `查看团队` 的输出方式，可选图片或文本 |
| `team_management.blacklist_provider` | `string` | 空 | `团队黑本` 使用的 AstrBot 对话模型；留空时使用当前会话的默认对话模型 |
| `team_management.blacklist_prompt` | `text` | 内置黑本提示词 | 与指定团队及其完整成员数据拼接后统一发送给模型，可在插件配置页编辑 |
...
团队规则、心法职责、会话控制、区服绑定、区服/心法别名、避雷记录、事件订阅及缓存配置均保存在本地 SQLite。修改 WebSocket 地址、事件版令牌或 TLS 证书验证开关后需要重新加载插件。
...
- `[参数]` 表示该参数可以不写，省略后采用默认值；指令表中的 `服务器` 或 `区服` 不使用方括号，其可省略条件以上一条会话绑定规则为准。
...
| `烟花 服务器 [角色] [条数]` | 查询指定服务器的烟花记录；角色可省略，条数默认为 50，非正整数或其他无效输入自动使用默认值；已绑定会话可省略服务器；图片 | Token |
...
| `跨服名剑 服务器 [模式]` | 跨服名剑队伍榜单，模式可选 `22`、`33`、`55`，分别映射接口模式 `0`、`1`、`2`，默认 `33`；无效输入使用默认值；图片 | Token（LV.2） |
| `武林争霸 服务器 [阵营]` | 武林争霸赛帮会榜，阵营可选 `浩气盟`、`恶人谷`，分别映射接口阵营 `1`、`2`，默认 `浩气盟`；无效输入使用默认值；图片 | Token（LV.2） |
| `捕快荣誉 服务器` | 捕快荣誉榜，展示角色、帮会、门派、阵营和抓捕数；图片 | Token（LV.2） |
| `江湖浪客 服务器` | 江湖浪客榜，展示角色、帮会、门派、阵营、抓捕数和敌对数；图片 | Token（LV.2） |
| `决斗挑战 服务器 [模式]` | 决斗挑战悬赏榜；当前入口实际统一按公开模式查询，模式参数无法切换到私密；图片 | Token（LV.2） |
...
`资历` 的 `1` 表示总览，`2-19` 固定对应杂闻、武学、修为、装备、技艺、阅读、任务、足迹、战斗、声望、秘境、帮会、阵营、节日、活动、风雨江湖路、家园和剑侠录，不再使用本地资历菜单和点数缓存。第二轮只需回复数字，不需要再次添加插件前缀；15 秒未选择时默认把首项 `{"1": "总览"}` 交给次轮查询并继续渲染。
...
| 权限 | 指令 | 说明 |
| --- | --- | --- |
| 群管理 | `开团 团名 人数 [公告]` | 在当前会话新建 10 人或 25 人团队并自动生成团队编号；新团默认关闭报名 |
| 群管理 | `打开报名 团编号` | 允许全员向指定团队报名 |
| 群管理 | `关闭报名 团编号` | 停止新增报名；同时兼容 `关闭报警 团编号` |
| 群管理 | `结束团队 团编号` | 删除指定团队及其全部报名，不受当前报名状态影响 |
| 群管理 | `结束全部团队` | 删除当前会话的全部团队及报名，不影响其他会话 |
| 群管理 | `清空报名 团编号` | 清空指定团队的成员并保留团队本身 |
| 群管理 | `修改报名 团编号 原角色名 新角色名 心法 [老板]` | 修改已有成员的角色名、心法和老板标记 |
| 群管理 | `交换位置 团编号 位置1 位置2` | 交换两个格子；其中一个为空时移动成员 |
| 群管理 | `查看限制 默认10\|默认25\|团编号` | 查看当前会话的默认规则或团队专属规则，例如 `查看限制 12` |
| 群管理 | `添加限制 范围 职责\|心法 目标 最小数量 最大数量` | 新增限制；职责目标支持 T、奶、DPS、老板 |
| 群管理 | `修改限制 范围 规则编号 职责\|心法 目标 最小数量 最大数量` | 修改指定限制规则 |
| 群管理 | `删除限制 范围 规则编号` | 删除指定限制规则 |
| 群管理 | `恢复默认限制 团编号` | 删除团队专属规则，恢复继承会话默认规则 |
| 群管理 | `团队黑本 团编号` | 将指定团队的完整成员数据与可编辑提示词发送给配置的模型，返回一名成员及理由 |
| 全员 | `报名 团编号 心法 角色名 [老板]` | 使用心法配置中的标准名称或别名报名到第一个空位；填写“老板”时显示特殊标记 |
| 全员 | `取消报名 团编号 角色名` | 从指定团队移除角色 |
| 全员 | `查看团队 [团编号]` | 默认图片输出，也可在插件配置中切换为文本；省略编号时展示全部团队，提供编号时展示团队详情 |

单个会话可以同时存在多个团队。开团时人数必填且仅支持 10 或 25，分别生成 2 个或 5 个五人小队（老板也计入上限）。团名最长 80 个字符，公告最长 500 个字符；同一团队内角色名不区分大小写且不能重复。报名心法必须匹配全局心法配置中的标准名称或别名，最终统一保存为标准名称。群管理指令允许平台群主、平台群管理员或 AstrBot 管理员使用。所有带团队编号的操作在该编号不存在或属于其他会话时都不会跨会话操作，而是返回当前会话的团队基本信息。`团队黑本` 同样仅允许群管理调用；指定模型为空时使用发送指令所在会话的默认对话模型，且不会把该次请求写入普通对话上下文。

WebUI 可以把所有标准心法全局划分为 `T`、`奶`、`DPS`。默认 T 心法为洗髓经、铁牢律、铁骨衣、明尊琉璃体；默认奶心法为云裳心经、补天诀、相知、灵素、离经易道；其余心法默认归为 DPS。恢复默认心法配置时会同时恢复内置别名和职责分类。

报名限制规则按会话隔离，每个会话分别具有 10 人和 25 人默认规则。规则分为职责和具体心法两类，职责可选 T、奶、DPS、老板；每项均可配置最小与最大数量。标记为老板的成员只占用老板名额，不占用其心法对应的 T、奶、DPS 名额或具体心法名额。最大数量达到后拒绝同类报名；最小数量表示在团队接近满员时提前保留足够空位。团队默认继承对应人数的会话规则；首次保存团队专属规则时复制当前默认规则，此后独立维护，也可恢复为继续继承默认规则。限制规则、成员修改和位置交换指令仅允许管理员操作，并且始终使用发送指令所在的当前会话，不接受会话 ID 参数。
...
避雷数据按 AstrBot 的 `unified_msg_origin` 严格隔离，一个会话无法查看或操作其他会话的记录。升级前已有的避雷记录会自动保留在普通会话不可访问的“历史公共数据”区。避雷功能没有会话内的用户角色校验；在“会话控制”放行的同一会话中，只要能够触发插件指令，当前实现就允许新增、修改和删除该会话的记录。如需限制到指定管理员，仍应在 AstrBot 或消息平台层配置权限。
...
AstrBot 插件详情页中的“剑网三插件管理”通过 Plugin Pages 桥接调用插件 Web API，不直接访问 Dashboard 凭据。当前共有八个页签：
...
| 心法配置 | 查看标准心法，维护全局 T/奶/DPS 职责及最多 5 个别名；配装 ID 不在页面显示，可恢复默认 |
| 团队管理 | 按会话筛选和创建 10/25 人团队；详情以每列 5 个的职责着色格子展示，支持折叠、成员编辑和拖拽换位；可打开或关闭报名、添加或移除报名，并维护会话默认及团队专属限制规则 |
...
页面顶部通过 JX3API `POST /token/stats` 展示当前 Token 的等级、已用次数、剩余次数和有效状态，成功结果在进程内保留 30 秒。WebUI 的全部区服选项直接读取本地 `server_aliases` 表中的标准区服，顶部“刷新数据”只重新读取管理页数据，不会请求 `/server/status/check`。插件初始化时仍会执行一次 `server_list()` 建立用于指令参数消歧的有效区服目录：缓存有效时读取接口缓存，缓存不存在或已过期时才请求上游。
...
接口内存缓存默认最多占用 16 MB，按 JSON 的 UTF-8 字节数统计并按最近最少使用顺序淘汰，WebUI 同时展示当前占用和缓存条数。SQLite 接口缓存独立按条数限制，默认最多 256 条；旧版接口条数配置会自动继承为 SQLite 上限。内存命中也会更新 SQLite 的访问时间，修改上限和插件启动时立即清理超额记录。SQLite 条数限制不是磁盘字节容量；删除记录后释放的页可供后续写入复用，数据库文件不一定立即缩小。图片二进制保存在 AstrBot 插件数据目录，默认总容量为 512 MB，超出后按最近最少使用顺序清理。每 60 秒自动清理过期接口和图片缓存，无需打开 WebUI；接口请求失败时不再返回过期数据。随机名片、随机语录、吃喝选择和随机贴吧等接口默认不缓存，避雷查看和避雷查询也默认不缓存最终图片；这些项目仍可在 WebUI 中显式覆盖。
...
4. 创建本地数据库、查询数据服务、团队服务、心法别名服务、会话控制服务、避雷服务、事件推送服务、WebUI 服务和消息构建器。

异步初始化阶段会连接数据库并创建以下本地表：

- `bilei`：按 AstrBot 会话保存避雷记录；升级前记录迁移到不可由普通会话访问的历史公共数据区。
- `raid_teams`：按 AstrBot 会话保存团队名称、人数上限、公告和报名状态。
- `raid_team_members`：保存团队的固定位置、心法、角色名、报名人和老板标记。
- `raid_team_rule_profiles`：保存每个会话的 10/25 人默认规则档及团队专属规则档。
- `raid_team_rules`：保存职责（T、奶、DPS、老板）和具体心法的最小/最大数量限制。
...
- `kungfu`：保存 JX3BOX 配装 ID、标准心法名、T/奶/DPS 职责及最多 5 个别名。
- `trade_item_cache`：JX3BOX 交易行基础物品数据缓存及更新时间。
- `cache_settings`：接口与图片指令的默认时间和单项覆盖配置。
- `cache_limits`：接口内存缓存总容量、SQLite 接口缓存最大条数及图片缓存总容量配置。
...
- `styles/pages/*.css`：可选，仅保留成本计算、成就、副本记录、沙盘等无法合理复用的复杂页面布局。当前 52 个页面中有 15 个需要专属 CSS。
...
| 文件 | 生命周期 | 内容 |
| --- | --- | --- |
| AstrBot 插件数据目录下的 `local_data.db` | 运行时创建和维护 | 团队与报名、心法与别名、避雷记录、会话控制、会话区服绑定、区服别名、事件订阅及基础数据缓存 |
...
`api_response_cache` 仅保存通过 JX3API 查询入口成功取得的原始 JSON；Token 和 Ticket 不写入缓存键或正文，凭据摘要只用于避免更换账号后误用旧缓存。同一缓存键的并发请求通过异步锁合并。读取顺序为“内存热缓存 → SQLite → 上游接口”；两级缓存使用同一项 TTL，内存按总字节数限制，SQLite 按记录条数限制。缓存过期后重新请求上游，权限拒绝、业务错误和网络失败均返回失败结果，不再使用旧数据兜底；强制刷新失败也会清除该请求原有的接口缓存。每 60 秒根据当前有效 TTL 自动清理过期接口记录，启动时同步淘汰历史超额记录；插件停用和初始化失败时回收清理任务。

图片缓存不会把二进制写入 SQLite。图片消息处理器会在请求业务数据前，使用当前指令名、完整有效参数、二轮选择项、必要的会话范围、静态资源签名和截图参数生成 SHA-256 键；静态资源签名按相对路径和文件内容计算，不受重新安装产生的文件修改时间变化影响。命中时直接把本地路径交给消息事件发送。未命中时才请求数据并渲染，再把 AstrBot 临时渲染结果复制到 `cache/images/`；SQLite 的 `image_render_cache` 只保存文件索引、大小、时间、最近访问记录和图文消息正文。超过 WebUI 配置的容量（默认 512 MB）后按最近最少使用顺序清理。
...
│   ├── team.py              # 会话隔离的团队、五人小队位置和报名数据服务
│   ├── team_manager.py      # 团队权限、参数规则、操作流程和文字结果实现
│   ├── team_rules.py        # 会话默认与团队专属报名限制、名额预留校验
...
└── templates/
    ...
    ├── pages/
    │   └── *.html           # 52 个页面内容与 Jinja2 数据绑定
    ├── styles/
    │   ├── tokens.css       # 设计变量与页面宽度
    │   ├── base.css         # 全局背景、外框和排版
    │   ├── components.css   # 表格、网格、卡片、状态等公共组件
    │   └── pages/*.css      # 可选的复杂页面独有布局（当前 15 个）
    ├── img/                 # 通用图片资源
    ├── sect/                # 门派与心法图标
    └── serendipity/         # 奇遇图标
```
...
4. 区服绑定 WebUI 的新增及行内编辑均只接受标准区服，聊天指令仍可使用区服别名；区服别名页展示完整标准区服目录并只允许行内编辑别名，心法配置页隐藏配装 ID，只允许修改已有心法的职责和别名。“恢复默认”使用页面内二次点击确认，确认后会用随包 JSON 完整覆盖对应数据库表，当前自定义配置无法从页面撤销恢复。
5. WebUI 的区服选项只使用本地区服别名表中的标准区服，打开或刷新页面均不会请求 `/server/status/check`；插件初始化时仍会优先读取该接口的缓存，缓存不可用才请求上游。WebUI 顶部 Token 统计最多每 30 秒请求一次 `/token/stats`。
