...
`api_response_cache` 仅保存通过 JX3API 查询入口成功取得的原始 JSON；Token 和 Ticket 不写入缓存键或正文，凭据摘要只用于避免更换账号后误用旧缓存。同一缓存键的并发请求通过异步锁合并。读取顺序为“内存热缓存 → SQLite → 上游接口”；两级缓存使用同一项 TTL，不存在单独的内存保留时间。内存超过 WebUI 配置的条数时只淘汰内存副本，后续仍可从 SQLite 读取。缓存过期后优先刷新，上游失败时允许使用七天内仍可解析的旧数据兜底。
